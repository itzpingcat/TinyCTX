...
Resolution: iterate sender's roles highest-position first, return first mapped
level, fall back to `default_permission`.

```python
def resolve_discord_level(member, bridge_cfg) -> int:
    role_map = bridge_cfg.options.get("role_permissions", {})
    default  = bridge_cfg.options.get("default_permission", 25)
...
| `bridges/discord/` | Read `role_permissions` + `default_permission` from options, resolve per sender |
