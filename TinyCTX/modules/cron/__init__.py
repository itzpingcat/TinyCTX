...
CronStore/_CronRunner are built once at @hook(HookType.STARTUP). add_cron/
list_cron/remove_cron need live cycle.caller/.context.tail_node_id/.db/
.config.permissions, which a @tool method has no way to receive, so they're
wired imperatively from @hook(HookType.TURN_START), same pattern as
modules/present.
"""
...
_ADD_CRON_DOC = """Schedule something to happen later, in this channel — a reminder,
...
class Cron(Module):
    """Scheduled agent turns. See this module's own docstring for the full design."""

    settings = {
        "store_file": {"default": "cron.db", "type": "str",
                       "description": "Path relative to config.data.path (NOT workspace)."},
        "poll_seconds": {"default": 20, "type": "float",
                          "description": "How often the scheduler polls the store for due jobs."},
    }
    ...
    def load(self, runtime) -> None:
        ...
        self._store = CronStore(data_path / store_file)

        poll_seconds = float(self.config["poll_seconds"])
        self._runner = _CronRunner(runtime, self._store, poll_seconds)
        self._runner.start()

        logger.info("[cron] background runner active")
    ...
    def wire_tools(self, cycle) -> None:
        ...
        if store is None or runner is None:
            logger.error("[cron] TURN_START before STARTUP — cron tools unavailable this turn")
            return
        ...
        def _current_channel() -> tuple[str, str] | None:
            ...
            """
            tail = cycle.context.tail_node_id if cycle.context else None
            if not tail or cycle.db is None:
                return None
            cursor_key = cycle.db.get_state(tail, "cursor_key", None)
            platform = cycle.db.get_state(tail, "platform", None)
        ...
        add_cron.__doc__ = _ADD_CRON_DOC
        ...
        def remove_cron(job_id: str) -> str:
            ...
            is_admin = caller.has_permission(Permission.CRON_ADMIN, cycle.config.permissions)
        ...
        cycle.tool_handler.register_tool(add_cron, required_permissions={Permission.CRON_CREATE})
        cycle.tool_handler.register_tool(list_cron, always_on=True, required_permissions=None)
        # remove_cron: ungated at the seam — see the ownership/CRON_ADMIN check
        # inside the function body above for why.
        cycle.tool_handler.register_tool(remove_cron, required_permissions=None)
