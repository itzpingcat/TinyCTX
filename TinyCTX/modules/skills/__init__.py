...
Discovery
---------
Rescanned periodically (rescan_interval_seconds) in @hook(HookType.PRE_ASSEMBLE_ASYNC),
off the event loop, for folders containing a SKILL.md file (skills) or a
DESCRIPTION.md file (categories). Categories can be nested arbitrarily deep.
Only frontmatter is parsed at discovery time — full content stays on disk
until activated.

Scan order (first-found wins for duplicate names):
  1. Paths listed in the skill_dirs setting, resolved relative to workspace
...
Both tools need cycle.tool_handler.enable() / cycle.context.db /
cycle.context.tail_node_id live at call time, which a @tool method has no
way to receive — they're registered imperatively from
@hook(HookType.TURN_START), same pattern as modules/present. Everything else
here — discovery, the skill index prompt, tag/drop tracking — only ever
touches ctx (the assembly Context each hook/prompt already receives) or this
module's own process-lifetime cache, so it stays plain @hook/@prompt.
"""
...
from TinyCTX.context import ROLE_TOOL
...
def _load_names(ctx, key: str) -> list[str]:
    try:
        raw = ctx.db.get_state(ctx.tail_node_id, key, "[]")
        return json.loads(raw)
    except Exception:
        return []


def _save_names(ctx, key: str, names: list[str]) -> None:
    try:
        ctx.db.set_state(ctx.tail_node_id, key, json.dumps(sorted(set(names))))
    except Exception as exc:
        logger.warning("[skills] failed to persist %s: %s", key, exc)


def _load_dropped(ctx) -> list[str]:
    return _load_names(ctx, _STATE_KEY_DROPPED)


def _save_dropped(ctx, names: list[str]) -> None:
    _save_names(ctx, _STATE_KEY_DROPPED, names)


def _load_expanded(ctx) -> set[str]:
    try:
        raw = ctx.db.get_state(ctx.tail_node_id, _STATE_KEY, "[]")
        return set(json.loads(raw))
    except Exception:
        return set()


def _save_expanded(ctx, expanded: set[str]) -> None:
    try:
        ...
        ctx.db.set_state(ctx.tail_node_id, _STATE_KEY, json.dumps(sorted(expanded)))
    except Exception as exc:
        logger.warning("[skills] failed to persist expanded categories: %s", exc)


class Skills(Module):
    """Agent Skills support (agentskills.io) — discovery, system-prompt
    index, use_skill/collapse_skill_categories tools."""

    settings = {
        "skill_dirs": {
            "default": ["skills"],
            "type": "list",
            "description": "Directories to scan for skills, in priority order, resolved against workspace.",
        },
        "index_priority": {"default": 5, "type": "int", "description": "System prompt priority (after soul, before memory)."},
        "dropped_footer_priority": {"default": 50, "type": "int", "description": "role=user priority for the dropped-skill reminder footer."},
        "ephemeral_categories": {"default": True, "type": "bool", "description": "If false, expanded categories persist across turns."},
        "rescan_interval_seconds": {"default": 30, "type": "float", "description": "Minimum time between filesystem rescans."},
        "tools": {
            "default": {"use_skill": "always_on"},
            "type": "dict",
            "description": "use_skill visibility: 'always_on' | 'deferred' | 'disabled'.",
        },
    }
    ...
    def load(self, runtime) -> None:
        self._ephemeral = bool(self.config["ephemeral_categories"])
        self._rescan_interval = float(self.config["rescan_interval_seconds"])

        workspace = Path(runtime.config.workspace.path).expanduser().resolve()
        configured: list[Path] = []
        for raw in self.config["skill_dirs"]:
            p = Path(raw)
        ...
        self._all_scan_dirs = configured + [d for d in convention_dirs if d not in configured]
        self._primary_dir = configured[0]
        configured[0].mkdir(parents=True, exist_ok=True)

        skills, categories, _ = self._refresh_sync()
        n_skills = len(skills)
        n_cats   = len(categories)
        if n_skills or n_cats:
            logger.info(
                "[skills] discovered %d skill(s), %d categor%s: skills=[%s] categories=[%s]",
                n_skills, n_cats, "y" if n_cats == 1 else "ies",
                ", ".join(sorted(skills.keys())), ", ".join(sorted(categories.keys())),
            )
        else:
            logger.info("[skills] no skills found yet — place skills in %s", self._primary_dir)

    def _refresh_sync(self) -> tuple[dict, dict, list]:
        """Blocking discovery scan. Only ever call this from a worker thread
        (via asyncio.to_thread) — never directly on the event loop."""
        skills, categories, top_level = _discover(self._all_scan_dirs)
        self._live["skills"]     = skills
        self._live["categories"] = categories
        self._live["top_level"]  = top_level
        self._live["last_scan"]  = time.monotonic()
        return skills, categories, top_level

    async def _refresh_async(self, force: bool = False) -> None:
        """Refresh the skills cache off the event loop. Safe to call from a
        PRE_ASSEMBLE_ASYNC hook every turn — it no-ops unless the cache is
        stale (or force=True), and the actual disk walk runs in a thread."""
        if not force and (time.monotonic() - self._live["last_scan"]) < self._rescan_interval:
            return
    ...
    async def pre_assemble_refresh(self, ctx) -> None:
    ...
    # is kept warm by pre_assemble_refresh above (awaited by the agent via
    # run_async_hooks(PRE_ASSEMBLE_ASYNC) BEFORE assemble() runs).

    @prompt(role="system", priority=5, name="skills_index")
    def build_prompt(self, ctx) -> str | None:
        top_level  = self._live["top_level"]
        categories = self._live["categories"]
        expanded = set() if self._ephemeral else _load_expanded(ctx)
        return _build_index_prompt(top_level, expanded, categories)

    # ------------------------------------------------------------------
    # Tools: use_skill, collapse_skill_categories — need live
    # cycle.tool_handler / cycle.context, so wired from TURN_START
    # ------------------------------------------------------------------
    ...
    def wire_tools(self, cycle) -> None:
        ...
        def use_skill(name: str) -> str:
            ...
            if name in categories:
                ...
                if not self._ephemeral:
                    expanded = _load_expanded(ctx)
                    expanded.add(name)
                    _save_expanded(ctx, expanded)
            ...
            if name in skills:
                ...
                if skill.tools:
                    ...
                    for tool_name in skill.tools:
                        (enabled if cycle.tool_handler.enable(tool_name) else unknown).append(tool_name)
                    note_parts = []
                    if enabled:
        ...
        sk_vis = str(self.config["tools"].get("use_skill", "always_on")).lower().strip()
        if sk_vis != "disabled":
            cycle.tool_handler.register_tool(
                use_skill,
                always_on=(sk_vis != "deferred"),
    ...
    @hook(HookType.TRANSFORM_TURN, priority=-100)
    def tag_use_skill_results(self, entry, age, ctx):
    ...
    @hook(HookType.POST_ASSEMBLE)
    def capture_dropped_skills(self, messages, ctx):
        ...
        resident = set(_load_names(ctx, _STATE_KEY_RESIDENT))
        ...
        _save_names(ctx, _STATE_KEY_RESIDENT, list(new_resident))

        if newly_dropped:
            existing = set(_load_dropped(ctx))
            _save_dropped(ctx, list(existing | newly_dropped))
        return None
    ...
    @prompt(role="user", priority=50, name="skills_dropped_footer")
    def dropped_skills_footer(self, ctx) -> str | None:
