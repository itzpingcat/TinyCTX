...
Hooks HookType.POST_COMPLETION (context.py), which fires in AgentCycle.run()
...
`liquid_notified`/`nudge_count` live on ctx.state (Context's own per-cycle
state bag, not dropped between assemble() calls within one AgentCycle) —
this hook is registered once per process and must not carry state itself,
but the nudge budget is genuinely per-cycle (a single incoming message can
trigger several completions before the turn finishes) and must never leak
across the unrelated sessions one TinyCTX process juggles concurrently, so
process-lifetime instance state on the module itself would be wrong too.
"""
...
class OutputParser(Module):
    """Detects tool calls emitted as text and nudges the model back onto its
    native tool-calling channel, or diagnoses a format a nudge can't fix."""
    ...
    settings = {
        ...
        "max_nudges_per_cycle": {
            "default": 2,
            "type": "int",
            "description": "Max nudges to issue per AgentCycle before giving up and falling "
                            "back to a single diagnostic notify instead.",
        },
    }

    @hook(HookType.POST_COMPLETION)
    def post_completion(self, response_text, tool_calls_list, ctx):
        ...
        state = ctx.state.setdefault("output_parser", {"liquid_notified": False, "nudge_count": 0})
        max_nudges = self.config["max_nudges_per_cycle"]
