EXTENSION_META = {
    ...
    "description": (
        ...
        "docs/PERMISSIONS-PLAN.md. user_modify_permissions sets a user's "
        "named permission template (permissions.templates in config.yaml); "
        "there is no more numeric ceiling logic since ROOT is total."
    ),
    "default_config": {
        # NOTE: model_min_permission is GONE — /model and set_active_model
        # are both gated on Permission.MODEL_SWAP (a template-granted bool),
        # not a configurable numeric threshold. See
        # docs/PERMISSIONS-PLAN.md §9's table.
    },
}
