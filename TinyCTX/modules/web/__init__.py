...
Registers web tools into each cycle's tool_handler:
...
One Camoufox (anti-detect Firefox) browser instance lives on the cycle for
the cycle's lifetime — which, since runtime.py constructs a fresh AgentCycle
per turn rather than reusing one per session, means (same as
modules/filesystem's file_read_state) it's actually per-turn today despite
reading like session-lifetime state; preserved exactly as-is, not "fixed".
It is created lazily on first use. All browser tools share that single page:
click, type_text, extract_* and screenshot_browser act on whatever open_url
loaded last.

Because the browser/settings state is per-cycle, every tool here needs live
cycle access at call time, which a @tool method has no way to receive — all
nine are wired imperatively from @hook(HookType.TURN_START), same pattern as
modules/present.
...
The pre-Module version also patched agent.reset() to close the browser on
reset — dead code, since AgentCycle has no reset() method (same class of
bug as modules/mcp's agent.reset() patching); not carried forward.
"""
...
# ---------------------------------------------------------------------------
# Per-cycle browser state (stored on the cycle instance)
# ---------------------------------------------------------------------------

_STATE_KEY = "_web_module"


def _state(cycle) -> dict:
    if not hasattr(cycle, _STATE_KEY):
        setattr(cycle, _STATE_KEY, {
            "camoufox":   None,
            "browser":    None,
            "page":       None,
            "settings": {
    ...
    return getattr(cycle, _STATE_KEY)


async def _ensure_page(cycle):
    """Lazily create a Camoufox browser page for this cycle."""
...
async def _close_browser(cycle) -> str:
    st = _state(cycle)
    try:
        if st["camoufox"]:
...
async def _locate(cycle, target: str, nth: int = 0, exact: Optional[bool] = None):
    page = await _ensure_page(cycle)
    t = target.strip()
...
async def _dynamic_discovery(cycle) -> list[dict]:
...
# Defaults: web_search and open_url are always_on; the rest are deferred.
# Can be overridden per-tool via settings.tools.<tool_name>: always_on|deferred|disabled
...
# docs/PERMISSIONS-PLAN.md §7. click/type_text are the interesting pair: a
# click on a nav link is a read, a click on a submit button is a write, and
# type_text puts locally-originated data into a remote form. Which one it is
# depends on the DOM, not the arguments — unlike shell's curl this isn't
# statically decidable, so it gets the conservative static answer
# (NETWORK_WRITE) rather than a per-call classifier.
...
class Web(Module):
    """Web tools: DuckDuckGo search, open_url, and Camoufox browser
    automation (click, type, extract, screenshot)."""

    settings = {
        "headless": {"default": "virtual", "type": "select",
                     "options": {"true": "", "false": "", "virtual": "Headful Firefox under Xvfb — survives bot checks."},
                     "description": "True | False | 'virtual'."},
        "timeout_ms": {"default": 30000, "type": "int", "description": ""},
        "settle_timeout_ms": {"default": 15000, "type": "int", "description": "Budget for settling past JS interstitials."},
        "screenshot_max_bytes": {"default": 1500000, "type": "int", "description": "Screenshots above this are saved but not shown inline."},
        "wait_until": {"default": "domcontentloaded", "type": "str", "description": ""},
        "shift_enter_for_newline": {"default": True, "type": "bool", "description": ""},
        "ignore_tags": {"default": ["script", "style"], "type": "list", "description": ""},
        "max_discovery_elements": {"default": 40, "type": "int", "description": ""},
        "browse_max_bytes": {"default": 2000000, "type": "int", "description": ""},
        "browse_max_chars": {"default": 20000, "type": "int", "description": ""},
        "browse_user_agent": {"default": "TinyCTX/1.1", "type": "str", "description": ""},
        "output_dir": {"default": "outputs/browser", "type": "str", "description": "Relative to workspace."},
        "tools": {
            "default": {},
            "type": "dict",
            "description": "Per-tool visibility override: {<tool_name>: 'always_on'|'deferred'|'disabled'}.",
        },
    }

    def resolve_settings(self, extra):
        # Deep-merge override — same reasoning as modules/system_prompt:
        # 'tools' is a nested dict a caller might partially override.
    ...
    def wire_tools(self, cycle) -> None:
        ...
        # user — created on demand at screenshot time instead.
        output_dir = workspace / cfg["output_dir"]
        ...
        st["settings"].update({
            ...
            "shift_enter_for_newline": cfg["shift_enter_for_newline"],
            "ignore_tags":            list(cfg["ignore_tags"]),
            "max_discovery_elements": cfg["max_discovery_elements"],
            "browse_max_bytes":       int(cfg["browse_max_bytes"]),
            "browse_max_chars":       int(cfg["browse_max_chars"]),
            "browse_user_agent":      str(cfg["browse_user_agent"]),
            "settle_timeout_ms":      int(cfg["settle_timeout_ms"]),
            "screenshot_max_bytes":   int(cfg["screenshot_max_bytes"]),
        })

        # Launch mode: True | False | "virtual" (Xvfb-backed headful — the default,
        # and the only mode that survives most bot checks).
        cfg_headless = cfg["headless"]
        if isinstance(cfg_headless, str):
            cfg_headless = cfg_headless.strip().lower()
            if cfg_headless in ("true", "yes", "1"):
                cfg_headless = True
            elif cfg_headless in ("false", "no", "0"):
                cfg_headless = False
            elif cfg_headless != "virtual":
                cfg_headless = "virtual"
        st["_headless"] = cfg_headless
        st["_default_headless"] = cfg_headless
        ...
        async def open_url(
            ...
            want_headless = st_local.get("_default_headless", "virtual") if headless is None else headless

            try:
                # If a browser is already open with a mismatched headless mode, close it.
                if st_local["browser"] is not None and st_local.get("_headless") != want_headless:
                    await _close_browser(cycle)
                st_local["_headless"] = want_headless

                page     = await _ensure_page(cycle)
                response = await page.goto(
                    url,
                    wait_until=st_local["settings"]["wait_until"],
                    timeout=st_local["settings"]["timeout_ms"],
                )
                status = response.status if response else 200
                stuck  = await _settle_navigation(page, st_local["settings"], wait_for=wait_for)
                stuck_note = (
                ...
                if mode == "screenshot":
                    path = _screenshot_path(st_local, filename)
                    if isinstance(path, str):
                        return path
                    await page.screenshot(path=str(path), full_page=True)
                    if stuck:
                        logger.warning("web: screenshot of %s taken on a likely interstitial", page.url)
                    return _image_result(path, inline, st_local["settings"]["screenshot_max_bytes"])

                if mode == "elements":
                    elements = await _dynamic_discovery(cycle)
                    return (
                ...
                if mode == "html":
                    ...
                    try:
                        ...
                        html = await page.content()
                        content = _html_to_text(html, st_local["settings"]["ignore_tags"])
                    title = await page.title() or ""
                content, truncated = _truncate_content(content, st_local["settings"]["browse_max_chars"])
        ...
        async def click(target: str, nth: int = 0, exact: bool | None = None) -> str:
            ...
            st_local = _state(cycle)
            try:
                loc = await _locate(cycle, target, nth=nth, exact=exact)
                await loc.wait_for(state="visible", timeout=st_local["settings"]["timeout_ms"])
                await loc.click(timeout=st_local["settings"]["timeout_ms"])
        ...
        async def type_text(
            ...
            try:
                ...
                await loc.wait_for(state="visible", timeout=st_local["settings"]["timeout_ms"])
                if clear:
                    await loc.fill("", timeout=st_local["settings"]["timeout_ms"])
                await loc.click(timeout=st_local["settings"]["timeout_ms"])
                ...
                if "\n" in text:
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if i < len(parts) - 1:
                            if st_local["settings"]["shift_enter_for_newline"]:
        ...
        async def extract_text(target: str = "", nth: int = 0, exact: bool | None = None) -> str:
            ...
            try:
                ...
                loc = await _locate(cycle, target, nth=nth, exact=exact)
                await loc.wait_for(state="attached", timeout=st_local["settings"]["timeout_ms"])
                return await loc.inner_text(timeout=st_local["settings"]["timeout_ms"])
            except Exception as e:
                return f"Error: {e}"
        ...
        async def extract_html(target: str = "", nth: int = 0, exact: bool | None = None) -> str:
            ...
            try:
                ...
                loc = await _locate(cycle, target, nth=nth, exact=exact)
                await loc.wait_for(state="attached", timeout=st_local["settings"]["timeout_ms"])
                return await loc.inner_html(timeout=st_local["settings"]["timeout_ms"])
            except Exception as e:
                return f"Error: {e}"
        ...
        async def wait_for(
            ...
            try:
                loc = await _locate(cycle, target, nth=nth, exact=exact)
                await loc.wait_for(state=state, timeout=st_local["settings"]["timeout_ms"])  # type: ignore[arg-type]
        ...
        async def manage_browser(action: str, key: str | None = None, value: str | None = None) -> str:
            ...
            if a == "list":
                ...
                return await _close_browser(cycle)

            elif a == "view_settings":
                return json.dumps({**st_local["settings"], "_headless": st_local.get("_headless", True)}, indent=2)

            elif a == "set_setting":
                if not key or value is None:
                    return "Error: set_setting requires both key and value."
                if key not in st_local["settings"]:
                    return f"Error: unknown setting '{key}'. Valid: {list(st_local['settings'].keys())}"
                current = st_local["settings"][key]
                if isinstance(current, bool):
                    st_local["settings"][key] = value.lower() in ("true", "1", "yes")
                elif isinstance(current, int):
                    st_local["settings"][key] = int(value)
                else:
                    st_local["settings"][key] = value
                return f"Setting '{key}' updated to {st_local['settings'][key]!r}."

            elif a == "add_ignore_tag":
                if not value:
                    return "Error: add_ignore_tag requires value."
                if value not in st_local["settings"]["ignore_tags"]:
                    st_local["settings"]["ignore_tags"].append(value)
                ...
                if value in st_local["settings"]["ignore_tags"]:
                    st_local["settings"]["ignore_tags"].remove(value)
        ...
        for fn in (
            ...
            vis = str(tools_cfg.get(fn.__name__, "")).lower().strip()
            if vis == "disabled":
                continue
