...
class Runtime:
    ...
    async def start(self) -> None:
        ...
        logger.info("Runtime started (%d models, %d tools)",
                    len(self.models), len(self.tool_handler._tools))
    ...
    def _maybe_write_checkpoint(self, node_id: str, msg: InboundMessage) -> None:
        ...
        self.db.update_node_content(node_id, self.db.get_node(node_id).content)  # no-op touch
        # Write checkpoint as state_delta on the user node we just created.
        # We can't update state_delta via update_node_content (it updates content).
        # Use a direct SQL update.
        self.db._conn.execute(
            "UPDATE nodes SET state_delta = ? WHERE id = ?",
            (json.dumps(checkpoint, ensure_ascii=False), node_id),
        )
